import BlueWordComponent from '../blueWordComponent';

function LyricsSilverDropsComponent() {
    return (
        <div><div class='songtitle'>SILVER DROPS</div><br />
            🌫️VERSE I🌫️<br />
            Em7.         	EbMaj7.    	Dm7<br />
            Silky silver <BlueWordComponent word='rain drops'/> on the ground<br />
            DbMaj7.      	Cm7.       	Cbm7<br />
            Sing your name all day, you're not around<br />
            Silky silver <BlueWordComponent word='rain'/> slides down the street                  <br />
            Always ask about my keys as I'm about to leave<br />
            🌧️CHORUS🌧️<br />
            EbMaj Ab/Bb<br />
            C#11 Ab11 Bb11<br />
            Do you hope i left them at your place<br />
            What would you do if that became the case?<br />
            Midnight mania coaxes us to play<br />
            in <BlueWordComponent word='shimmering'/> truth's illusion lies the chase<br />
            <br />
            VERSE II🌫️<br />
            Softly drifting winks of fragrant <BlueWordComponent word='snow'/><br />
            Want to leave this aaaaaahhh can't let go<br />
            Slyly drifting winks betwixt the scent<br />
            Our smooth bare skin warm in your bed<br />
            <br />
            🌧️CHORUS🌧️<br />
            Oooohh but we could wait, it's not too long<br />
            <BlueWordComponent word='Ambrosia'/>-moments <BlueWordComponent word='néctars'/>-never late<br />
            Mmmm <BlueWordComponent word='honey'/>-'slow sensuous <BlueWordComponent word='taste'/><br />
            In <BlueWordComponent word='shimmering'/> truth's illusion lies the chase<br />
            <br />
            VERSE III🌫️<br />
            Silky silver <BlueWordComponent word='rain'/> suspended tease<br />
            Ask me once once more before i leave<br />
            I can't promise you I'll stay<br />
            But forever <BlueWordComponent word='drips'/> in silver <BlueWordComponent word='drops'/> of <BlueWordComponent word='rain'/><br />
            PSYCHOUTRO🌨️<br />
            I can't let go (🔁)<br />
            Silky silver rain<br />
            Sing all day<br />
            Mmmm honey-'slow<br />
            illusion winks<br />
            Shimmers truth<br />
            Come play<br />
        </div>
    )
}
export default LyricsSilverDropsComponent